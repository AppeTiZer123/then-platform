...
export async function POST(
  ...
  try {
    ...
    // Add Response
    const responseData = await consultationRepo.addResponse(params.id, {
      responderId: session.user.id,
      responderName: session.user.name || "เจ้าหน้าที่",
      message,
    });

    return NextResponse.json({ ok: true, response: responseData }, { status: 201 });
  } catch (error) {
    console.error(`POST /api/admin/consultations/${params.id}/reply error:`, error);
    return NextResponse.json(
