import { pgSchema, uuid, varchar, text, boolean, timestamp, integer, decimal, date, jsonb } from 'drizzle-orm/pg-core';
...
export const fraudAccounts = thenApp.table('fraud_accounts', {
  ...
  totalDamage: decimal('total_damage', { precision: 15, scale: 2 }).default('0'),
...
export const reports = thenApp.table('reports', {
  ...
  damageAmount: decimal('damage_amount', { precision: 15, scale: 2 }).default('0'),
  suspectFraudAccountId: uuid('suspect_fraud_account_id').references(() => fraudAccounts.id, { onDelete: 'set null' }),
  suspectPhone: varchar('suspect_phone', { length: 20 }),
  suspectSocialMedia: text('suspect_social_media'),
  status: varchar('status', { length: 20 }).default('pending'),
  assignedOfficerId: uuid('assigned_officer_id').references(() => officers.id, { onDelete: 'set null' }),
...
export const consultationResponsesRelations = relations(consultationResponses, ({ one }) => ({
